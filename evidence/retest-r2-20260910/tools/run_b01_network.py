"""B-01 子项复测：A06 断网注入（进程级网络沙盒）+ 名称解析失败形态（S4-R2）。
...
名称解析失败形态由 `run_b01_dns.py` 在重启候选后单独注入（不经代理直连保留域）。

S4-R2：S4-R1 时该形态因候选启动期的 Responses 端点校验直接退出而受阻；该校验已随
B-03 剥离删除，本批次须取得区别于前三种的解析阶段失败证据。
"""
...
from r2common import (
    WS_URL, EXE, REPO, case_dir, load_token, save_result, sidecar_log_mark,
    sidecar_tail, write_info_log, write_commands, write_http_log, result_shell,
)
...
async def run_injection(session: Harness, project_id: str, label: str,
    ...
    created = await session.call("conversation.create", {
        "project_id": project_id, "pair_id": PAIR_A, "title": f"R2-B01-{label}"},
        timeout=30)
...
async def main() -> int:
    ...
    session = Harness(WS_URL, token, frames_log=out / "ws-frames.log",
                      events_log=out / "events.log", device_name="r2-network")
    ...
    try:
        ...
        conversation = await session.call("conversation.create", {
            "project_id": project_id, "pair_id": PAIR_A, "title": "R2-B01-warmup"},
            timeout=30)
    ...
    write_commands(out / "commands.ps1", [
        "# 1) 启动沙盒",
        "python evidence/retest-r2-20260910/tools/net_sandbox.py --listen 8767 "
        ...
        "# 3) 注入",
        "python evidence/retest-r2-20260910/tools/run_b01_network.py",
    ])
    result["status"] = "待判定"
...
if __name__ == "__main__":
    if os.environ.get("R2_DNS_ONLY") == "1":
        from b01_dns import main_dns
