...
审批模式旁提供角色对话思考档位下拉框，可选“自动、低、高、最大”，默认“低”，选择同样按项目保存。审查智能体复用该角色对话模型配置。
...
> 真正暂停执行的一方是 app-server（approval policy + sandbox），
...
| 模式 | 运行位置 | 审批行为 |
|---|---|---|
| 请求批准 | 沙箱内 | app-server 在沙箱边界发出执行前审批请求时挂起，用户选择允许、本对话内允许或否决；沙箱内可直接执行的常规只读动作不会产生审批 |
| 帮我审核 | 沙箱内 | 低风险操作直接放行；高风险操作由审查智能体裁决允许或否决 |
| 完全允许运行 | 沙箱外 | 直接操作，不拦截；工具与审批事件照常持久化 |

“本对话内允许”按“工具类型 + 操作签名”缓存，当前聊天结束后失效。

高风险判定使用全局规则表（与搭档无关的配置文件），覆盖删除、批量覆盖、git 破坏性命令、网络访问和依赖安装等类别。命中规则的操作连同当前聊天中用户最后发送的 3 条消息一起交给审查智能体。提示词要求它先检查用户是否直接要求或明确批准了当前操作，再结合操作风险裁决。审查智能体只能读取这些用户消息和待审批操作，输出结构化的允许或否决；否决时附一句简短理由和调整建议，回给助手用于修改方案。审查智能体不能修改文件、调用执行工具、创建子助手或继续委派，模型配置复用当前 `DialogueModel` 适配器。
...
| 职责 | 承担方 | 说明 |
|---|---|---|
| 真正暂停执行 | app-server | thread/start 配置 `approvalPolicy` 与 `sandbox` 后，越过当前边界的操作会在执行前挂起并发起 `requestApproval` 请求 |
...
|---|---|---|---|
| 请求批准 | `"untrusted"` | `"read-only"` | `"user"` |
| 帮我审核 | `"untrusted"` | `"read-only"` | `"user"`（应用层审查智能体裁决） |
...
操作越过当前沙箱边界前挂起并发起审批请求，应用层 `ApprovalManager` 裁决后经
`resolve_approval` 回复，实现真实差异化拦截；完全允许运行保持
`"workspace-write"` 直接执行。目录级兜底校验（§14.2）仍保留。

`"untrusted"` 是当前 app-server 可用的最强原生执行前审批策略：不受信任命令和写操作会请求审批，受信任的常规只读命令可直接运行。应用层不再在 `tool.started` 之后补发无法阻止执行的审批卡片。
