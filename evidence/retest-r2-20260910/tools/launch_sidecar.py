...
def main() -> int:
    ...
    parser.add_argument(
        "--issue-code",
        action="store_true",
        help="启动后经 stdin 桌面通道请求一个配对码（S4-R2：隔离实例需要与远程连接对话）",
    )
    parser.add_argument("--issue-delay", type=float, default=12.0)
        ...
        if args.issue_code:
            def issue() -> None:
                ...
                try:
                    ...
                    print(f"issue_code failed: {exc!r}", flush=True)

            threading.Thread(target=issue, name="issue-code", daemon=True).start()
