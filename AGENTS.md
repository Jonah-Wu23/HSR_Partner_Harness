...
真实浏览器会话列表布局回归（Playwright，使用模拟数据）：
...
发布节奏：发布版本时重新生成并上传 NSIS 安装包；v0.4.1 同时发布 Android arm64 Debug APK。安装包构建必须经过 `desktop/scripts/tauri-with-first-run-reset.ps1` 完成首次运行数据清理。
